  ...
  try {
    ...
    const cleanPrice =
      typeof productPrice === "string"
        ? Number(productPrice.replace(/,/g, ""))
        : productPrice;
    if (!productName || !productDesc || !cleanPrice || !category || !brand) {
...
export const getProductById = async (req, res) => {
  try {
    const { id } = req.params; // This 'id' comes from the URL /:id
    ...
    return res.status(200).json({
      success: true,
      product, // This is the 'product' object your frontend is waiting for
    });
  } catch (error) {
    return res.status(500).json({
      success: false,
